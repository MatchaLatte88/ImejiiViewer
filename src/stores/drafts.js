...
import { deleteDraft, listDrafts, loadDraft, saveDraft, projectBlob, readProject } from '../lib/drafts.js'
...
export const useDraftStore = defineStore('drafts', () => {
  ...
  const canExport = computed(() => ui.mode === 'logo' ? editor.hasImage : library.hasItems)
  ...
  async function restore(id) {
    ...
    try {
      ...
      if (entry.kind === 'photo') { if (await library.restoreDraft(entry)) ui.setMode('images') }
  ...
  async function exportProject(id) {
    ...
    try {
      await flush()
      const entry = await loadDraft(id)
      await saveBlob(await projectBlob(entry.file, entry.kind, entry.state), slugify(entry.name) + '.imejii')
  ...
  function dispose() { stop?.(); stop = null; clearTimeout(timer) }
