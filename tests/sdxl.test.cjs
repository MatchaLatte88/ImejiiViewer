  ...
  const server = http.createServer(async (req, res) => {
    try {
      ...
      if (req.url.startsWith('/object_info/')) {
        ...
        if (name === 'KSampler') { required.sampler_name = [['euler']]; required.scheduler = [['normal']] }
...
test('SDXL parameters, buffers and fixed workflow reject free model/path/operation inputs', () => {
  for (const change of [{ model: '../model' }, { prompt: '' }, { steps: 51 }, { seed: -1 }, { cfg: NaN }, { denoise: .01 }]) assert.throws(() => parameters({ ...params, ...change }))
  ...
  assert.equal(graph['7'].inputs.sampler_name, 'euler'); assert.equal(graph['1'].inputs.ckpt_name, MODEL)
  ...
  assert.equal(WORKFLOWS.outpaint, 'imejii-sdxl-base-outpaint-v1'); assert.equal(SDXL_SIZES.length, 9)
})
